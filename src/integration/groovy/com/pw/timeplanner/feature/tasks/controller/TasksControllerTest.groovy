...
class TasksControllerTest extends UserInitializedSpecification {
    ...
    @Autowired
    ProjectsTestRepository projectsTestRepository
    ...
    def "should create task"() {
        ...
        then:
            ...
            createdTask.with {
                ...
                assert it.projectId == defaultProject
            }
    ...
    def "should get task"() {
        given:
            def task = tasksRepository.save(Task.builder()
                                                .name("test")
        ...
        then:
            response.andExpect(status().isOk())
                    ...
                    .andExpect(jsonPath('$.projectId', equalTo(task.projectId.id.toString())))
    ...
    def "should update task's project"() {
        given:
            def project = projectsTestRepository.save(Project.builder().name("Project 1").userId(USER_ID).build())
            def task = tasksRepository.save(Task.builder()
                                                .name("test")
                                                .projectId(defaultProject)
                                                .userId(USER_ID)
        ...
        cleanup:
            projectsTestRepository.delete(project)
    }
    ...
    def "should return error when trying to update task with invalid data"() {
        ...
        then:
            ...
            tasksRepository.findById(task.id).orElseThrow().with {
                ...
                assert it.projectId.id == task.projectId.id
