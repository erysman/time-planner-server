...
class ProjectControllerTest extends UserInitializedSpecification {
    ...
    def "should get all projects"() {
        given:
            def project = projectsRepository.save(Project.builder().name("Project 1").userId(USER_ID).build())
        when:
    ...
    def "should throw when trying to create project with existing name"() {
        given:
            def project = projectsRepository.save(Project.builder().name("Project1").userId(USER_ID).build())
    ...
    def "should delete project and it's tasks"() {
        given:
            def project = projectsRepository.save(Project.builder().name("Project 1").userId(USER_ID).build())
            tasksService.createTask(USER_ID, CreateTaskDTO.builder().name("Task 1").projectId(project.id).build())
            tasksService.createTask(USER_ID, CreateTaskDTO.builder().name("Task 2").projectId(project.id).build())
        when:
        ...
        then:
            ...
            tasksService.getTask(USER_ID, task.id)
            tasksService.getTask(USER_ID, task2.id)
    }

    def "should not be able to delete default project, should just delete all it's tasks"() {
        given:
            def task = tasksService.createTask(USER_ID, CreateTaskDTO.builder().name("Task 1").projectId(defaultProject.id).build())
            def task2 = tasksService.createTask(USER_ID, CreateTaskDTO.builder().name("Task 2").projectId(defaultProject.id).build())
        when:
    ...
    def "should update project"() {
        given:
            def project = projectsRepository.save(Project.builder()
                    .name("Project1")
                    .userId(USER_ID)
